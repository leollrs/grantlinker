...
const ShaderCanvas = () => {
  ...
  useEffect(() => {
    ...
    updateColor();
    // For our always-dark site, force dark
    setBackgroundColor([0.043, 0.059, 0.078]); // #0B0F14
...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    ${isPopular ? 'scale-105 relative ring-2 ring-emerald-400/20 from-white/20 to-white/10 border-emerald-400/30 shadow-2xl' : ''}
  `;
  const buttonClasses = `
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-emerald-500 text-white">
          Más popular
        </div>
      )}
      <div className="mb-3">
        <h2 className="text-[32px] md:text-[40px] font-extralight tracking-[-0.03em] text-white font-display">{planName}</h2>
        <p className="text-[14px] text-white/60 mt-1 font-sans">{description}</p>
      </div>
      <div className="my-5 flex items-baseline gap-1">
        <span className="text-[36px] md:text-[44px] font-extralight text-white font-display">{price}</span>
      </div>
      <div className="w-full mb-5 h-px bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.09)_20%,rgba(255,255,255,0.22)_50%,rgba(255,255,255,0.09)_80%,transparent)]" />
      <ul className="flex flex-col gap-2 text-[14px] text-white/80 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-emerald-400 w-4 h-4" /> {feature}
          </li>
        ))}
      </ul>
...
export const ModernPricingPage = ({
  ...
  return (
    <div className="relative w-full overflow-hidden">
      ...
      <div className="relative w-full flex flex-col items-center justify-center px-4 py-16 md:py-24">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          <h1 className="text-[36px] md:text-[56px] font-extralight leading-tight tracking-[-0.03em] bg-clip-text text-transparent bg-gradient-to-r from-white via-emerald-300 to-blue-400 font-display">
            {title}
          </h1>
          <p className="mt-3 text-[16px] md:text-[18px] text-white/60 max-w-2xl mx-auto font-sans">
            {subtitle}
          </p>
        </div>
