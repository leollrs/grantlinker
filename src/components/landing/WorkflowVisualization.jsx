...
export default function WorkflowVisualization() {
  ...
  return (
    <div className="relative">
      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#111827] to-[#0F172A] p-10 backdrop-blur">
        <div className="space-y-5">
          {modules.map((module, i) => {
            const Icon = module.icon;
            return (
              <div key={i}>
                <div className="flex items-center gap-4 rounded-xl border border-white/5 bg-white/[0.02] p-5 hover:border-emerald-500/20 transition-all group">
                  <div className="rounded-lg bg-emerald-500/10 p-3 group-hover:bg-emerald-500/20 transition-all">
                ...
                {i < modules.length - 1 && (
                  <div className="flex justify-center py-2">
                    <div className="h-6 w-px bg-gradient-to-b from-emerald-500/50 to-transparent" />
                  </div>
                )}
              </div>
            );
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
          <p className="text-[13px] text-[#9AA6B2]">Sistema operativo completo</p>
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-emerald-500" />
            <p className="text-[13px] text-emerald-400 font-medium">En línea</p>
          </div>
        </div>
      </div>
